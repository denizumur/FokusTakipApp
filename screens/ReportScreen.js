...
export default function ReportScreen() {
  ...
  const processData = (sessions, categoryFilter) => {
    ...
    const pData = Object.keys(categoryMap).map((cat, index) => ({
      ...
      legendFontColor: theme === 'dark' ? '#ccc' : '#555',
      legendFontSize: 12
    }));
    setPieData(pData);
  ...
  useFocusEffect(useCallback(() => { fetchSessions(); }, [theme]));
  ...
  const chartConfig = {
    ...
    decimalPlaces: 0,
    color: (opacity = 1) => theme === 'dark' ? `rgba(0, 243, 255, ${opacity})` : `rgba(37, 99, 235, ${opacity})`,
